<section class="space-y-4">
    <div>
        <h1 class="text-2xl font-semibold">Επεξεργασία Δρόμου</h1>
    ...
    <article class="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <form action="<?= e(url('/attachments/upload')) ?>" method="post" enctype="multipart/form-data" class="mb-3 flex flex-wrap items-center gap-2">
            ...
            <input type="hidden" name="redirect_to" value="<?= e(url('/roads/' . $road['id'] . '/edit')) ?>">
    ...
    <form action="<?= e(url('/roads/' . $road['id'] . '/delete')) ?>" method="post" onsubmit="return confirm('Διαγραφή δρόμου;');">
        <?= csrf_field() ?>
        <button class="rounded-md bg-rose-700 px-4 py-2 text-sm text-white">Διαγραφή Δρόμου</button>
    </form>
</section>
